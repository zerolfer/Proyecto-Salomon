...
public class ThreadEntorno3 implements Runnable {
	...
	public ThreadEntorno3(List<Solucion> listaSoluciones,int i, Solucion individuo1, int tmnIntervalo, Patrones patrones, Entrada entrada, Parametros parametros, ParametrosAlgoritmo_SA parametrosAlg) {
			...
			if(i<j || tmn == individuo1.getControladores().size()) {
				...
				for (int k = 0; k < c1.length()-tmnIntervalo*3; k+=3) {
					...
					if(!intervalo.replaceAll("111", "").equalsIgnoreCase("") && intervalo2.replaceAll("111", "").equalsIgnoreCase("")) { //c1:NoVacio c2:Vacio
						if(k!=0) {ant=c2.substring(k-3, k);}else {ant="";}
						if(k+(3*tmnIntervalo)!=c1.length()) {pos=c2.substring(k+(3*tmnIntervalo), k+(3*tmnIntervalo)+3);}else {pos="";}
						//REGLA 3
