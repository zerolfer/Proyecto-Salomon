...
public class FitnessFase3Info {
				...
				if (!str.equals("111")) {
					...
					if(j+1==numeroFranjasHorarias){
						if(tPosicion!=0){
							...
							if(Math.abs(tPosicion - parametros.getTiempoTrabOpt())>=15){c++;}
							if(Math.abs(tPosicion - parametros.getTiempoTrabOpt())>=20){d++;}
							if(Math.abs(tPosicion - parametros.getTiempoTrabOpt())>=25){e++;}
							x++;
							tPosicion = 0;
						}
					}
			...
			for(int j=0;j<=numeroFranjasHorarias;j++){
				if(j==numeroFranjasHorarias){
					if(!strPrevia.equals("111")){
						...
						if(Math.abs(tPosicion - parametros.getTiempoPosOpt())>=10){c++;}
						if(Math.abs(tPosicion - parametros.getTiempoPosOpt())>=15){d++;}
						if(Math.abs(tPosicion - parametros.getTiempoPosOpt())>=25){e++;}
						x++;
					}
				}else{
					str = tabla.get(i).substring(j*3, j*3+3);
					...
					}else if(!str.equals("111")){tPosicion = parametros.getTamanoSlots();}
					strPrevia = str;
				}
			}
			strPrevia=null;
	...
	public static double tiempoPorcentaje(int numeroControladores, int numeroFranjasHorarias, ArrayList<String> tabla, Parametros parametros) {
		...
		for (int i = 0; i < numeroControladores; i++) {
			...
			if(diferencia <parametros.getPorcentPosMin()){
				...
				x+=diferencia;
				if(diferencia>0.05){c++;}if(diferencia>0.1){d++;}if(diferencia>0.15){e++;}
			}
	...
	public static double maximizarAcreditacion(Entrada entrada, int numControladores, int numTurnos, ArrayList<String> turnos, double sectoresElementalesTotales) {
		...
		for (int i = 0; i < turnos.size(); i++) {
			...
			ArrayList<Sector> sectoresTurno = new ArrayList<Sector>();
			...
			for (int j = 0; j < sectoresTurno.size(); j++) {
				...
				for (int k = 0; k < sectoresEl.size(); k++) {
					...
					if(!contain){elementales.add(sectorEl);}
				}	
			}
