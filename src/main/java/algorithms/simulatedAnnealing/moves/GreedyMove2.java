...
public class GreedyMove2 {
                ...
                if (!ant.equals("111")) {
                    if (l + 3 < turno.length() && ant.equals(turno.substring(l + 3, l + 6))) {
                        ...
                        if (cont < minT - 1) { //No se cumple el trabajo minimo
                            ...
                            contM++;
                            i -= 1;
                            cont = 0;//Reseteamos l=-1 (al iniciar el bucle de nuevo l++ =0) para volver a comprobar el turno despues de los cambios
                            break;
                        }
                        cont = 0;
                    }
    ...
    private static int[] busquedaAdquisicion(Entrada entrada, Parametros parametros, ArrayList<String> turnos,
        ...
        int[] re = {-1, -1, -1};
        int iniCad = finCad;
        ArrayList<String> dosTurnos = new ArrayList<>();
        ArrayList<String> dosTurnosViejos = new ArrayList<>();
        ...
        for (int i = 0; i < turnos.size(); i++) {
            ...
            if (num1 >= num2 && num1 != -1) {
                if (prev) {
                    ...
                    re[2] = iniCad + 3;//Posicion turno + Posicion inicio + Posicion fin
                } else {
                    re[0] = i;
                    re[1] = finCad;
                    re[2] = finCad + slotsNecesarios + 3;//Posicion turno + Posicion inicio + Posicion fin
                }
                return re;
            }
        }
            ...
            if (i != posTurno) {
                ...
                if (num1 >= num2 && num1 != -1) {
                    if (prev) {
                        re[0] = i;
                        re[1] = iniCad;
                        re[2] = finCad + 3;//Posicion turno + Posicion inicio + Posicion fin
                    } else {
                        re[0] = i;
                        re[1] = iniCad;
                        re[2] = finCad + 3;//Posicion turno + Posicion inicio + Posicion fin
