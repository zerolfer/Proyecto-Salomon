...
public class SimulatedAnnealing {
	 ...
	 */
	public static ArrayList<Solucion> bucleSA(ArrayList<Solucion> poblacionInicial,ParametrosAlgoritmo_SA pa, Parametros p, Patrones patrones, Entrada entrada){
		/*
		...
		for (int i = 0; i < MainPruebas.tiemposPorSolucionAux.length; i++) {
			...
			mediaAux += a;
			if(maxAux<a) {maxAux = a;}
			if(minAux>a) {minAux = a;}
		}
	 ...
	 */
	public static Solucion simulatedAnnealing(Solucion individuo,int n, ParametrosAlgoritmo_SA parametrosAlg, Parametros parametros, Patrones patrones, Entrada entrada) {
		/*
		...
		double rand=0;double empeora = 0;double aMejor= 1;
...
//			iteracion.add(""+iteracionActual);//ciclo
			/*Trazas2*/
//			Solucion individuo2 = DeciderMove.switchMoves(individuo,gMax,gMin, 0, parametrosAlg, patrones, entrada, parametros, iteracion);
			Solucion individuo2 = DeciderMove.switchMoves(individuo,gMax,gMin, 0, parametrosAlg, patrones, entrada, parametros, new ArrayList<String>());
			if(individuo.getTurnos().size()!=individuo.getControladores().size()){
				individuo2= orderByLazyCriteria(individuo2);
			}
			
			//fitIteraciones1 = DeciderFitnessFunction.switchFitnessF(individuo, patrones, entrada, parametros, parametrosAlg);
			fitIteraciones2 = DeciderFitnessFunction.switchFitnessF(individuo2, patrones, entrada, parametros, parametrosAlg);
...
//					trazaFit=trazaFit+(bestFit+"").replace(".",",")+";"+(val2+"").replace(".",",")+";"+(fitIteraciones2[1]+"").replace(".",",")+";"+(fitIteraciones2[2]+"").replace(".",",")+";"+(fitIteraciones2[3]+"").replace(".",",")+";"+(fitIteraciones2[4]+"").replace(".",",")+";"+(temperatura+"").replace(".", ",")+"\n";
				}else{
					individuo2= null;
//					trazaFit=trazaFit+(bestFit+"").replace(".",",")+";"+(val1+"").replace(".",",")+";"+(fitIteraciones1[1]+"").replace(".",",")+";"+(fitIteraciones1[2]+"").replace(".",",")+";"+(fitIteraciones1[3]+"").replace(".",",")+";"+(fitIteraciones1[4]+"").replace(".",",")+";"+(temperatura+"").replace(".", ",")+"\n";
				}
			}else if(val1<val2){
				if(bestFit<val2){
					bestFit = val2;
...
//				iteracion.add("0");//Cambia?
//			}
			if(cutOff && m==1000) {temperatura = temperatura * descensoTemp;}
			/*Trazas3*/
			iteracionActual++;
			if(iteracionActual%iteracionesTemp==0){
				if(formulaDesc.equalsIgnoreCase("Clasica")) {
					...
					temperatura = parametrosAlg.getTemperaturaInicial() /(1+descensoTemp*Math.log(1+iteracionActual));
				}else if(formulaDesc.equalsIgnoreCase("Adaptativa")){
					temperatura = 0.15 * (1-bestFit+ temperatura - ((double)iteracionActual/500000.0)); //elevado a la 1. TODO: PRUEBAAAAAA!
					if(temperatura<0) {temperatura=0.00000000000001;}
...
//				System.out.println(pem);
				...
				MainPruebas.problema +=("TempAct: "+ temperatura+"| "+"Iteracion: "+ iteracionActual +" |Tamaño Ind: " +individuo.getTurnos().size() + " |Porct. de Mejora: "+ porcentMejora+ "% o N. Mejoras: "+aMejor+ " |FITAnt .. FITBest: "+ fitAnt +" .. "+ bestFit + " |tiempo: "+ (((ti2-ti1)/1000.0)/60.0)+"\n");
			//	System.out.print("TempAct: "+ temperatura+"| ");
			//	System.out.println("Iteracion: "+ iteracionActual +" |Tamaño Ind: " +individuo.getTurnos().size() + " |Porct. de Mejora: "+ porcentMejora+ "% o N. Mejoras: "+aMejor+ " |FITAnt .. FITBest: "+ fitAnt +" .. "+ bestFit + " |tiempo: "+ (((ti2-ti1)/1000.0)/60.0));
...
//			iteracion = new ArrayList<String>();
		...
		if(res == 0) {
			...
			if(MainPruebas.TiemposResultadosproblema[1]==0) {
				MainPruebas.TiemposResultadosproblema[1] = (((System.currentTimeMillis()-MainPruebas.TiemposResultadosproblema[0])/1000.0)/60.0);
			}
		}
		/*Trazas5*/
...
//		Trazas.getTrazas2().add(Main.Main.carpetaTrazas+"/trazas-"+parametrosAlg.getFuncionFitnessFase2()+"-Fit"+n+"_"+dateFormat.format(new Date())+".csv");
