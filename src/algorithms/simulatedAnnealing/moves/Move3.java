...
public class Move3 {
	...
	private static int TurnoAleatorio(ArrayList<Integer> contc, ArrayList<String> individuo) {
		int c = (int) (Math.floor(Math.random() * ((individuo.size()-1)+ 1)));
		boolean noEsta = true;
		while(noEsta){
			noEsta = false;
				...
				while(!encontrado && listG.size()!=0){
					...
					if(granularidad>=tmnIntervalo) {
						for (int i = 0; i < listG.size(); i++) {
							if(tmnIntervalo<=listG.get(i)) {listG.remove(i);}
						}
					}
	...
	private static ArrayList<Integer> crearListG(int longdescansos,int granularidadMax) {
		...
		}if(longdescansos==4||longdescansos==8||longdescansos==12){
			for(int i=12;i<=granularidadMax;i+=12){listG.add(i);}
		}else if(longdescansos==5||longdescansos==10){
			for(int i=15;i<=granularidadMax;i+=15){listG.add(i);}
		}else if(longdescansos==0){
			for(int i=3;i<=granularidadMax;i+=3){listG.add(i);}
		}else{
