...
public class Move2 {
	...
	private static ArrayList<String> ChangeCont(Solucion ind,Movimiento mov, Patrones patrones, Entrada entrada, Parametros parametros, ParametrosAlgoritmo_SA parametrosAlg) {
		...
		boolean m = comprobarNucleos(controladorVago2, controladorP2, patrones);
		if(t || !m){
			return null;
		}
	...
	public static Solucion movimientoTrabajoIntervalosAleatorios(Solucion individuo1, int granularidadMax, int granularidadMin, Patrones patrones, Entrada entrada, Parametros parametros, ParametrosAlgoritmo_SA pa, ArrayList<String> iteracion){
		/*
		...
		while(contcv.size()<matrizTurnos.size() && !encontrado){
			...
			if(intervalos.size()!=0){
				...
				for(int i=granularidadMin;i<=granularidadMax;i+=3){listG.add(i);}
